...
%       .pooledSensitivity [1 x numClasses] pooled sensitivity/recall
%       .pooledSpecificity [1 x numClasses] pooled specificity
%
...
% a full 85-image loop's timing/behavior.
    ...
    pooledSensitivity = nan(1, numClasses);
    pooledSpecificity = nan(1, numClasses);
    for ch = 1:numClasses
        ...
        denomSensitivity = pooledTP(ch) + pooledFN(ch);
        if denomSensitivity > 0
            pooledSensitivity(ch) = pooledTP(ch) / denomSensitivity;
        end

        denomSpecificity = pooledTN(ch) + pooledFP(ch);
        if denomSpecificity > 0
            pooledSpecificity(ch) = pooledTN(ch) / denomSpecificity;
        end
    end
    ...
    results.pooledSensitivity = pooledSensitivity;
    results.pooledSpecificity = pooledSpecificity;
    ...
    for ch = 1:numClasses
        ...
        fprintf('%-8s  Dice: %.4f   IoU: %.4f   Sensitivity: %.4f   Specificity: %.4f   N=%d/%d%s\n', ...
            classNames{ch}, pooledDice(ch), pooledIoU(ch), ...
            pooledSensitivity(ch), pooledSpecificity(ch), nEvaluated(ch), nImages, note);
    end
end
