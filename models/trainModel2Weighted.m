...
load('D:\Projects\dr-screening\models\model2_fusion_untrained.mat', 'net2');  % fresh, not the overfit checkpoint

% inverse-frequency class weights, from your real training-split counts
trainCounts = [1362 276 830 210 252];   % grades 0-4
numClasses = 5;
classWeights = numel(trainIdx2) ./ (numClasses * trainCounts);


lossFcn = @(Y,T) crossentropy(Y, T, classWeights, ...
    'NormalizationFactor', 'all-elements', 'WeightsFormat', 'C') * numClasses;

mkdir('D:\Projects\dr-screening\checkpoints_model2_weighted');
...
commonOptsCell = { ...
    ...
    'CheckpointPath', 'D:\Projects\dr-screening\checkpoints_model2_weighted', ...
...
net2trained = trainnet(dsTrain2, net2, lossFcn, options);

save('D:\Projects\dr-screening\models\model2_final_weighted.mat', 'net2trained', '-v7.3');
fprintf('Weighted Model 2 training complete. Saved to model2_final_weighted.mat\n');