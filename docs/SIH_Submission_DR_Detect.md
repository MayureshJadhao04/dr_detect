...
*Note on Proliferative channel: The low metrics reflect extreme data scarcity in open-source annotations (only 49 positive validation patches, 5,842 positive pixels total across 479 training images). Because specificity is 100.0%, it does not produce false positives, and any positive activation triggers the clinical safety escalation rule.
