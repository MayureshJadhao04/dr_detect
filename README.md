...
[![Clinical Sensitivity](https://img.shields.io/badge/Referable%20DR%20Sensitivity-91.4%25-success.svg)](#-clinical-performance--metrics)
[![Referable Specificity](https://img.shields.io/badge/Referable%20DR%20Specificity-88.7%25-blue.svg)](#-clinical-performance--metrics)
[![Quadratic Kappa](https://img.shields.io/badge/Quadratic%20Kappa-0.871-orange.svg)](#-clinical-performance--metrics)
[![Deployment](https://img.shields.io/badge/Deployment-Fully%20Offline%20Edge-059669.svg)](#-quickstart--deployment)

> **DR_Detect** is a clinical-grade, offline-first diabetic retinopathy (DR) screening system engineered for primary healthcare centers and rural tele-ophthalmology clinics. Combining deep multi-lesion segmentation (DeepLabv3+), early-fusion multi-class severity grading (ResNet-101), multi-modal Grad-CAM explainability, and automated single-page A4 diagnostic PDF reports, DR_Detect enables accredited nurses and healthcare workers to identify sight-threatening retinopathy in under 15 seconds without cloud reliance.
...
### Headline Validation Benchmark (Held-out Split, $N=550$ Images)

| Clinical Diagnostic Metric | DR_Detect Score | Standard Benchmark | Clinical Meaning |
| :--- | :---: | :---: | :--- |
| **Referable DR Sensitivity (Grade $\ge$ 2)** | **91.4%** | >90.0% (SIH PS Target) | **Detects 91.4% of all sight-threatening DR cases needing specialist intervention** |
| **Referable DR Specificity (Grade $\ge$ 2)** | **88.7%** | >85.0% (SIH PS Target) | **Avoids flooding tertiary eye hospitals with non-urgent / healthy patients** |
| **Quadratic Weighted Kappa ($\kappa$)** | **0.871** | >0.80 (Substantial Agreement) | Strongly penalizes severe inter-grade misclassifications |
| **AUC-ROC (Referable DR)** | **0.938** | >0.900 (Clinical Grade) | Area under receiver operating characteristic curve |
| **Overall 5-Class Exact Accuracy** | **83.2% – 84.6%** | ~78–82% (ResNet-50) | Exact 5-way integer class match across all ICDR levels |
...
* **Exact vs Triage Scoring:** A model predicting Grade 2 for a patient with Grade 1 receives $0\%$ credit under strict 5-class accuracy, but for clinical screening triage, **both Grade 0 and Grade 1 are Non-Referable**, and **Grades 2, 3, and 4 are Referable**. Because our model's misclassifications occur almost exclusively between adjacent grades ($\kappa = 0.871$), its **triage sensitivity remains exceptional at 91.4%**.
...
graph TD
    ...
    subgraph Engine ["MATLAB AI Engine (pipelineServer.m)"]
        ...
        F --> G[4-Channel Lesion Probability Tensor]
        E --> H[resizeWithPad RAW Aspect Letterboxing]
        G --> I[Binarization & Aspect Letterboxing]
        H & I --> J[Early-Fusion Tensor: 224x224x7]
        J --> K[Model 2: Dual-Branch ResNet-101 Classifier]
        K --> L[ICDR Grade 0-4 + Softmax Confidence]
        K --> M[Grad-CAM Feature Gradient Extraction]
        M --> N[Radial Boundary Feathering]
        L & N --> O{Grade >= 2 OR Conf < 65%?}
...
│   ├── model2_final_weighted.mat # Trained 7-channel ResNet-101 weights
│   ├── trainModel1.m             # Model 1 masked BCE training routine
│   ├── trainModel2Weighted.m     # Model 2 inverse-frequency weighted training
│   ├── DRPatchDatastore.m        # Custom datastore for 256x256 patch sampling
│   ├── DRClassificationDatastore.m # Custom datastore for 7-channel fusion inputs
...
│   └── dist_electron/            # Compiled Windows installer (DR-Detect-1.0.0-x64.exe)
...
* Place `model1_final.mat` (60.2 MB) and `model2_final_weighted.mat` (163.8 MB) inside the `models/` directory.
...
```matlab
% 1. Harmonize dataset into 224x224 aspect-padded images
addpath('data_prep');
...
% 3. Train Model 2 (7-Channel ResNet-101 Fusion)
run('models/trainModel2Weighted.m');

% 4. Run master benchmark evaluation to reproduce metrics
...
```
The resulting installer `DR-Detect-1.0.0-x64.exe` is generated in `desktop/dist_electron/`.
